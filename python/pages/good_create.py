...
class Page(BasePage):
    ...
    def oncreate(self):
        ...
        if not name:
            self.error('Наименование должно быть задано')
            return
        ...
        group_value_name = group_param.get_name(self.postgres, group_value)
    ...
    def row(self, table, id, name, options = None, placeholder=None):
        row = table.row(id)
        row.cell().text(name)
        if options:
            ...
            if placeholder:
                Input(row.cell(), name=id, placeholder=placeholder)
            else:
    ...
    def __call__(self):
        ...
        self.row(table, 'code', 'Код'.upper(), placeholder='<АВТОНУМЕРАЦИЯ>')
