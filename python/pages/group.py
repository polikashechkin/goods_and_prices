...
class Page(BasePage):
    ...
    def onchange(self):
        ...
        if value:
            #if param.id == 'unit_id':
            #    value = Good.Unit.get(value)
            #    value_name = value.short_name
            #elif param_id == 'country_id':
            #    value = Good.Country.get(value)
            #    value_name = value.sname
            #else:
    ...
    def __call__(self):
        ...
        #------------------------------------------
        for param in self.postgres.query(GoodParam).filter(GoodParam.disabled==False).all():
            row = table.row()
